...
<body>
    ...
    <div class="centerize-box">
        <div class="rounded-box" style="width:1200px; display:block; margin-bottom:5%; margin-top:0;">
            <h1 class="title-font"> Admin Records</h1>
            <p class="general-font"> To update or delete admin data, click on its respective ID.</p>
            <div style="display:flex; margin-top:10px;">
                <table>
                    <thead class="schedule">
                        <th class="scheduledata" colspan="1">Admin ID</th>
                        <th class="scheduledata" colspan="2">Admin Name</th>
                    </thead>
                    <tr>
                    @foreach($staffTable as $staffsTable)
                        <td class="scheduledata"><a  style="text-decoration: none; color: inherit;" href="{{route('updateadmin',  ['staffTableID' => $staffsTable->staffID])}}">{{$staffsTable->staffID}}</a></td>
                        <td class="scheduledata" colspan="2">{{ $staffsTable->staffName }}</td>
                    </tr>
                    @endforeach
                    
                </table>
            </div>
            <center><button class="Login"><a href="{{url('addAdmin')}}">Add New Staff</a></button></center>
        </div>
    </div>
