...
<body>
    ...
    <div class="centerize-box">
        <div class="rounded-box" style="width:1200px; display:block; margin-bottom:5%; margin-top:0;">
            <h1 class="title-font">Manage Doctors</h1>
            <p class="general-font"> To update or delete doctor's data, click on its respective ID.</p>
            <div style="display:flex; margin-top:10px;">
                <table>
                    <thead class="schedule">
                        <th colspan="1" class="scheduledata">Doctor ID</th>
                        <th colspan="2" class="scheduledata">Doctor Name</th>
                    </thead>
                    <tr>
                    @foreach($doctor as $doctors)
                        <td class="scheduledata"><a  style="text-decoration: none; color: inherit;" href="{{route('updatedoctor', ['doctorID' => $doctors->doctorID])}}">{{$doctors->doctorID}}</a></td>
                        <td class="scheduledata" colspan="2">{{ $doctors->doctorName }}</td>
                    </tr>
                    @endforeach
                    
                </table>
            </div>
            <center><button class="Login"><a href="{{url('adddoctor')}}">Add New Doctor</a></button></center>
        </div>
    </div>
