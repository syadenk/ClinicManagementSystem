...
<body>
    ...
    <div class="centerize-box">
        <div class="rounded-box" style="width:1200px; display:block; margin-bottom:5%; margin-top:0;">
            ...
            <p class="general-font"> To update or delete patient data, click on its respective ID.</p>
            <div style="display:flex; margin-top:10px;">
                <table>
                    <thead class="schedule">
                        <th class="scheduledata" colspan="1">Patient ID</th>
                        <th class="scheduledata" colspan="2">Patient Name</th>
                    </thead>
                    <tr>
                    @foreach($patient as $patients)
                        <td class="scheduledata" ><a  style="text-decoration: none; color: inherit;" href="{{route('updatepatients', ['patientID' => $patients->patientID])}}">{{$patients->patientID}}</a></td>
                        <td class="scheduledata" colspan="2">{{ $patients->patientName }}</td>
                    </tr>
                    @endforeach
                    
                </table>
            </div>
            <center><button class="Login"><a href="{{url('addpatient')}}">Add New Patient</a></button></center>
        </div>
    </div>
