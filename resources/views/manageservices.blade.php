...
<body>
    ...
    <div class="centerize-box">
        <div class="rounded-box" style="width:1200px; display:block; margin-bottom:5%; margin-top:0;">
            <h1 class="title-font">Manage Services</h1>
            <p class="general-font"> To update or delete clinic services, click on its respective ID.</p>
            <div style="display:flex; margin-top:10px;">
                <table>
                    <thead class="schedule">
                        <th class="scheduledata" colspan="1">Service ID</th>
                        <th class="scheduledata" colspan="2">Service Name</th>
                    </thead>
                    <tr>
                    @foreach($service as $services)
                        <td class="scheduledata"><a  style="text-decoration: none; color: inherit;" href="{{route('updateservices', ['serviceID' => $services->serviceID])}}">{{$services->serviceID}}</a></td>
                        <td colspan="2" class="scheduledata">{{ $services->serviceName }}</td>
                    </tr>
                    @endforeach
                    
                </table>
            </div>
            <center><button class="Login"><a href="{{url('addservices')}}">Add New Service</a></button></center>
        </div>
    </div>
